...
class 合并数据集(Dataset):
    def __init__(self, 数据列表, 最大长度=128):
        ...
        # 使用本地分词器
        self.分词器 = BertTokenizer.from_pretrained('D:/PythonProject/LLM/bert-base-chinese')
...
def 初始化模型(标签数量):
    ...
    try:
        ...
        model = BertForSequenceClassification.from_pretrained(
            'D:/PythonProject/LLM/bert-base-chinese',
            num_labels=标签数量,
...
# 6. 保存模型函数
def 保存模型(model, 分词器, 准确率, 标签数量, 数据量, 训练历史):
    """保存训练好的模型"""
    print("\n=== 保存模型 ===")
...
def 主函数():
    ...
    if 准确率 > 0:
        print(f"\n🎉 训练完成！准确率: {准确率:.2%}")
        模型路径 = 保存模型(model, 分词器, 准确率, 标签数量, 数据量, 训练历史)
        print(f"\n✅ 模型已保存到: {模型路径}")
        print("   包含文件:")
